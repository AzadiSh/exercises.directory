...
reset; set term svg mouse standalone; set out "-2(x+1)^4+4_gnuplot.svg"
...
array point[1]
plot \
"+" u 1:(-2*($1+1)**4) @lconf @p1color, \
"" u 1:(-2*($1+1)**4)+3 @lconf @p2color
...
set label at -1,3 "{/:Italic (-1, 3)}" offset -1,1

plot for [p1p = p1fx : p1lx] point us (p1p):(-2*((p1p+1)**4)) @dConf @p1color
plot for [p1p = p1fx : p1lx] point us (p1p):(-2*((p1p+1)**4)+3) @dConf @p1color
